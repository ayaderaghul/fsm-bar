...
(define (build-random-population n state#)
  (define v (build-vector n (lambda (_) (make-random-automaton state#))))
  (cons v v))
...
(define (match-up-d population0 rounds-per-match delta gamma)
  (define population (car population0))
  ...
  (for ([i (in-range 0 (- (vector-length population) 1) 2)])
    ...
    (define-values (round-results a1 a2) (interact-d p1 p2 rounds-per-match delta gamma))
    (vector-set! population i a1)
    (vector-set! population (+ i 1) a2))
  population0)

(define (match-up-c population0 rounds-per-match delta gamma)
  (define population (car population0))
  (population-reset population)
  (for ([i (in-range 0 (- (vector-length population) 1) 2)])
    (define p1 (vector-ref population i))
    (define p2 (vector-ref population (+ i 1)))
    (define-values (round-results a1 a2) (interact-c p1 p2 rounds-per-match delta gamma))
    (vector-set! population i a1)
    (vector-set! population (+ i 1) a2))
  population0)
